...
RSpec.describe 'User show page:', type: :feature do
  before(:each) do
    @user1 = User.new(name: 'unique_show', photo: 'http://localhost:3000/anything.jpg', bio: 'Anything test', posts_counter:0)
    @user1.save
    ...
    @post3 = Post.new(title: 'post three', text: 'post three text', author: @user1, comments_counter:0, likes_counter:0)
  ...
  scenario 'I can see the users first 3 posts' do
    visit user_path(@user1.id)
    expect(page).to have_content(@user1.recent_posts.first.title)
    expect(page).to have_content(@user1.recent_posts.second.title)
    expect(page).to have_content(@user1.recent_posts.third.title)
  end
  ...
  #  visit user_path(@user1.id)
  #  click_link("Post ##{@user1.recent_posts.first.id}")
  #  expect(page).to have_current_path(user_post_path(@user1.id,@user1.recent_posts.first.id))
  # end
